...
## 20. 2026-06-08 Real 模式全量 UAT 与最终收口

### 20.1 验收范围

本轮使用当前 `.env` 中的 Real 模式配置，执行真实模型、真实业务数据库、真实联网检索、Skill 编排、MCP 调用、FastAPI/SSE 和前端生产构建验收。测试过程中不记录任何密钥、密码、主机地址等敏感信息。

### 20.2 Real MCP 全量健康检查
...
| MCP | 结果 | 说明 |
|---|---|---|
| `llm` | PASS | 真实模型连接与回答正常 |
| `n2sql` | PASS | 真实模型生成 SQL 正常 |
| `sql_executor` | PASS | 真实数据库只读查询正常 |
| `knowledge_retrieval` | PASS | 当前 `DIFY_ENABLED=false`，验证本地知识降级链路 |
| `web_search` | PASS | 真实联网检索正常 |
| `time` | PASS | 时间工具正常 |
| `text_analysis` | PASS | 文本分析正常 |

结论：**7/7 MCP PASS**。

### 20.3 多问题、多 Skill Real UAT

| 编号 | 测试问题/场景 | 期望 Skill | 实际 Skill | 关键 MCP | 结果 |
|---|---|---|---|---|---|
| REAL-UAT-01 | 汽车保养周期简要问答 | `chat` | `chat` | `llm` | PASS |
| REAL-UAT-02 | 2023 年 6 月中东公司批发量和终端量 | `data_query` | `data_query` | `llm`、`knowledge_retrieval`、`n2sql`、`sql_executor` | PASS，批发量 `2160`、终端量 `2311` |
| REAL-UAT-03 | 2023 年 6 月中东公司终端量同比 | `yoy_yoy_analysis` | `yoy_yoy_analysis` | `knowledge_retrieval`、`n2sql`、`sql_executor`、`llm` | PASS，去年同期为 `0` 时未生成无效百分比 |
| REAL-UAT-04 | 联网搜索中东汽车市场趋势 | `web_search_answer` | `web_search_answer` | `web_search`、`llm` | PASS |
| REAL-UAT-05 | 内部销量与公开市场信息综合分析 | `data_web_compare_analysis` | `data_web_compare_analysis` | `llm`、`knowledge_retrieval`、`n2sql`、`sql_executor`、`web_search` | PASS |
| REAL-UAT-06 | 基于上文继续与竞品对比 | `web_compare_analysis` | `web_compare_analysis` | `llm`、`web_search` | PASS，复用上文数据且未重复查询内部数据库 |
| REAL-UAT-07 | 隐式全球市场表现问题 | `data_web_compare_analysis` | `data_web_compare_analysis` | 内部数据链路 + `web_search` | PASS |
| REAL-UAT-08 | 联网开关关闭时明确要求联网 | 无联网 Skill | 无联网 Skill | 不调用 `web_search` | PASS，返回开启联网搜索提示 |

结论：**核心 6 类 Skill 与联网开关规则全部通过 Real 端到端验收**。

### 20.4 本轮发现并解决的问题

跨表问题 `2023年6月中东公司的批发量和产量分别是多少？` 首次 Real UAT 暴露排产表字段配置错误：

- 原激活主题 DDL 使用不存在的 `scheduling_qty`，真实数据库稳定返回字段无法解析错误。
- 直接 Real SQL 验证确认真实字段为 `product_qty` 和 `delivery_qty`。
- 修正 `theme_config.json`、`theme_loader.py` 和本地知识字段说明。
- SQL 纠错 Prompt 现在同时接收真实活动主题表结构，避免执行失败后继续猜测字段。
- 新增回归测试锁定排产表真实字段和 SQL 纠错表结构注入。
...
库存边界问题 `2023年6月中东公司的库存量是多少？` 工作流和 SQL 均成功，但数据库返回 `None`。该结果属于指定日期范围内的数据可用性，不是系统执行错误。

### 20.5 Real API/SSE 验收

使用明确要求批发量与终端量的问题调用真实 `POST /api/chat/stream`：

| 验收项 | 结果 |
|---|---|
| HTTP 状态 | `200` |
| SSE 事件 | `message_created`、`step_started`、`step_completed`、`result_ready`、`answer_delta`、`answer_completed` 全部出现且顺序正确 |
| 步骤持久化 | PASS，保存 `8` 个真实步骤 |
| Skill 持久化 | PASS，`data_query` |
| 已知业务值 | PASS，最终回答包含批发量 `2160`、终端量 `2311` |
| 工作流错误 | 无 |
...
| 项目 | 最终结果 |
|---|---|
| 全量自动化测试 | PASS，`67 passed, 1 warning` |
| Real MCP 健康检查 | PASS，`7/7` |
| 核心 Skill Real UAT | PASS，`6/6` |
| 复杂与边界问题 | PASS；库存空值按数据可用性记录 |
| Real API/SSE | PASS |
| 前端生产构建 | PASS，`tsc -b && vite build` |
| `git diff --check` | PASS |
...
| 编号 | 最终状态 | 关闭依据 |
|---|---|---|
| OPEN-03：核心视觉与原型反馈 | CLOSED | 浏览器反馈已修正并完成截图验收 |
| OPEN-04：Real 模式 UAT | CLOSED | 本轮真实模型、数据库、联网、Skill、MCP、API/SSE 全量验收通过 |
| OPEN-05：真实运行中 SSE | CLOSED | callback 异步队列和前端实时步骤消费已完成 |
| OPEN-06：结构化 ECharts | CLOSED | Markdown 数值表格已生成响应式 ECharts，并完成浏览器验收 |
| OPEN-07：细粒度生命周期事件 | CLOSED | 已实现并验证 `step_started`、`result_ready`、`answer_delta` |
| 多指标/跨表 N2SQL | CLOSED | 真实“批发量 + 产量”问题已修复并通过 |

### 20.8 最终结论与非阻塞说明

**当前生产 Web 范围内的已知阻塞问题和历史 OPEN 项均已解决并完成验收。**

以下事项不是当前功能阻塞：

- `DIFY_ENABLED=false`：远程 Dify 知识库未启用，本轮验证的是 `knowledge_retrieval` 本地知识降级链路；后续若启用远程 Dify，需单独做远程连通性验收。
- Python 3.9 LibreSSL 与 LangGraph PendingDeprecation warning：不影响当前测试与运行结果，可在后续环境升级中处理。
- 前端构建存在大 chunk 提示：构建成功，不影响当前功能；后续可通过动态加载优化首屏资源体积。
- 个别日期/指标查询返回 `None`：属于真实数据库数据可用性，应由业务数据治理处理。

## 21. 2026-06-08 流式渲染、自动跟随与 ECharts 稳定性收口

### 21.1 用户 UAT 反馈与根因

| 问题 | 根因 | 修复 |
|---|---|---|
| Markdown 标题、分隔线和表格排版异常 | 流式中间态与最终结果使用两套渲染路径，且表格块解析范围过大 | 统一为单一 `ResultRenderer`；补齐标题、分隔线、Markdown 清理和连续表格分块 |
| 流式输出持续闪烁 | 每个 `answer_delta` 触发前端二次打字机，并反复重建结果节点和 ECharts | 移除实际渲染链路中的 `TypewriterResult`；SSE 增量直接追加至消息内容；ECharts 实例只初始化一次 |
| 流式输出时页面不跟随到底部 | 原跟随逻辑只监听消息数组变化，无法可靠监听渲染后真实内容高度变化 | 使用 `ResizeObserver` 监听消息内容容器；仅在流式期间跟随；用户向上滚动后暂停跟随 |
| 点击“思考过程”后自动跳到输出末端 | 浏览器原生 scroll anchoring 在折叠内容高度变化时擅自调整滚动位置 | 为 `.message-list` 设置 `overflow-anchor: none`，由应用完全控制滚动 |
| ECharts 在流式过程中或完成后消失 | 表格解析合并多个表格，且图表实例随增量内容反复销毁重建 | 每个连续表格独立解析和绘图；图表实例与数据更新解耦；Top10 表格优先使用非数值维度列作为横轴 |

### 21.2 浏览器实测证据

在真实页面 `http://127.0.0.1:5174/` 对“广汽国际 2024 年 Top10 销量国家”结果进行验证：

| 验收项 | 实测结果 |
|---|---|
| ECharts | PASS，页面存在 `1` 个 ECharts canvas，展开/折叠思考过程前后均保留 |
| 多表格解析 | PASS，页面独立渲染 `2` 个表格，未再合并 |
| 单一流式渲染路径 | PASS，`result-renderer=2`，`typewriter-result=0` |
| 空段落与异常大间距 | PASS，空段落为 `0` |
| 思考过程展开/折叠滚动稳定性 | PASS，内容高度从 `3992` 变为 `3612`，消息区 `scrollTop` 变化量为 `0` |
| 浏览器滚动锚定 | PASS，`.message-list` 计算样式为 `overflow-anchor: none` |

### 21.3 回归保护

- 契约测试锁定单一结果渲染路径、直接消费 SSE `answer_delta`、稳定 ECharts 实例、多表格解析和流式跟随逻辑。
- 契约测试锁定 `.message-list` 必须禁用浏览器原生 scroll anchoring。
- 用户向上滚动会暂停自动跟随；新流式消息开始时恢复跟随。

### 21.4 最终状态

本轮用户反馈的排版异常、流式闪烁、不自动跟随、思考过程跳底和 ECharts 消失问题均已完成代码修复、契约测试和浏览器验收，当前状态为 **CLOSED**。
