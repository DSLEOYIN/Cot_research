...
- 管理端目前仍主要基于前端 mock 数据驱动，不是真实统一资产 API
- 详情页已接入统一生命周期状态工具，但工作台与统一目录仍未完全复用同一套状态入口
- 统一目录与工作台使用的是原型级任务/资产映射，仍需进一步收敛到统一状态工具
...
- [x] 将 Skill/MCP 详情页的阶段逻辑抽到统一状态工具
  ...
  - 需要统一：
    ...
    - 当前阶段动作文案
    - 风险摘要与阶段焦点配置
- [ ] 将工作台、统一目录、详情页全部改为复用同一套生命周期状态工具
- [ ] 接入真实统一资产 API
  - 后端新增统一资产接口
...
- [ ] 启动前端 preview 时自动提示 mock API 依赖
- [ ] 在登录页增加环境状态提示
  - mock API 是否可用
  - 当前是否为 mock 演示模式
- [ ] 为登录失败增加更清晰的错误分类
  ...
  - 后端不可用
  - 权限不足
- [ ] 将“平台管理员 / 销售员工 / AI 开发者”演示账号逻辑与后端 mock 数据一一对齐
- [ ] 增加管理端页面基础 smoke test
  - 登录
  - 打开工作台
  - 打开统一目录
  - 打开 Skill 详情
...
- [ ] 给统一目录补充分组视图
  ...
  - 按风险分组
- [ ] 为统一目录增加“仅看我的待处理”开关
- [ ] 为统一目录增加“最近失败原因”高亮样式优化
- [ ] 为统一目录增加空状态与无结果状态
- [ ] 为统一目录补充分页或虚拟列表能力，避免未来数据量大时卡顿

### 6.4 P1：工作台完善

- [ ] 将工作台的任务面板与统一资产状态彻底对齐
- [ ] 移除工作台中仍偏“平台概览”的统计项
- [ ] 增加“最近我操作过的对象”区块
- [ ] 增加“审核退回待补充”专门分组
- [ ] 增加“依赖解锁后恢复测试”的可视反馈
- [ ] 工作台时间线改为可跳转至对象详情而不只是选中抽屉

### 6.5 P1：Skill / MCP 详情页深化

- [x] Skill/MCP 详情页的阶段骨架组件共用
- [ ] 把 Skill/MCP 详情页的摘要卡、阶段焦点卡再继续组件化
- [ ] 统一 Skill 与 MCP 页面的“当前阶段主操作”交互密度
- [ ] 将详情页中的测试区与最近记录区拆成可复用子组件
- [ ] 为 Skill 详情页增加真实的“提审资料完整性检查” mock
- [ ] 为 MCP 详情页增加真实的“Schema 变更差异” mock
- [ ] 让详情页中的“发布前治理配置”跳转到运行与权限页的对应位置
- [ ] 为阻塞态详情页提供专门的解除阻塞引导

### 6.6 P1：发布流水线与运行与权限收尾

- [ ] 将发布流水线中的卡片点击统一跳回对象详情主操作
- [ ] 将发布流水线中的数据模型切换到统一资产视角
- [ ] 保留跨对象总览，但减少旧运营型表述
- [ ] 将运行与权限页拆成更清晰的四个视图
  - 组织树
  - 角色模板
...
- [ ] 为工作台路由解析补单元测试
- [ ] 为统一目录排序补单元测试
- [ ] 增加后端 mock API 的统一资产接口测试
- [ ] 增加浏览器级回归测试
- [ ] 将构建告警与 chunk 体积纳入定期检查项

### 6.9 P2：性能与工程化

- [ ] 拆分 `echarts` 相关大包，降低首屏 chunk
