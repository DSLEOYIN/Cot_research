...
export function SkillShowcasePage({ skill, onNavigate, onInstall, onTry }: Props) {
  ...
  return <section className="skill-showcase-page">
    ...
    <section className="showcase-requirements">
      <div className="requirements-intro"><span>BEFORE YOU START</span><h2>安装前检查</h2><p>完成下面检查后，这个 Skill 才能正确访问所需数据并返回结果。</p><button className="primary-action" type="button" onClick={trySkill}>{skill.installed ? '进入智能问答体验' : '安装到我的助手'}</button></div>
